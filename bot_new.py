...
class GymLegendBot:
    ...
    def handle_command(self, user_id, username, command):
        ...
        if cmd == 'к' and cmd_args:
            ...
            elif cmd_args[0] == 'распределить' and len(cmd_args) >= 2:
                args = ' '.join(cmd_args[1:])
                return self.clan_distribute_command(user_id, args)
    ...
    # ======================
    # ОСТАЛЬНЫЕ КОМАНДЫ КЛАНОВ (без изменений)
    # ======================
    ...
    def upgrade_clan_command(self, user_id):
        ...
        if result['success']:
            ...
            clan_bonuses = self.get_clan_bonuses(result['new_level'])
    ...
    def clan_treasury_command(self, user_id):
        ...
        log_text = ""
        for entry in log:
            action_emoji = "➕" if entry['action_type'] == 'deposit' else ("⬆️" if entry['action_type'] == 'upgrade' else ("💰" if entry['action_type'] == 'lift_income' else ("🏦" if entry['action_type'] == 'business_income' else "📊")))
        ...
        response_text = (
            ...
            f"💰 Казна: <b>{format_number(clan['treasury'])} монет</b>\n"
            f"👥 Участников: <b>{len(members)}</b>\n"
            f"📈 Доход/час: <b>{format_number(clan['total_income_per_hour'])} магнезии</b>\n\n"
            f"🎯 <b>Бонусы клана (Новая система):</b>\n"
            ...
            f"└─ 🏋️ +{clan_bonuses['lift_bonus_coins']} монет игроку за поднятие\n\n"
            f"🏆 <b>Топ вкладчиков:</b>\n{members_text}\n"
            f"📜 <b>Последние операции:</b>\n{log_text}\n"
            f"💡 <i>Положить деньги: /к положить [сумма]</i>\n"
            f"💡 <i>Распределить казну: /к распределить всем [сумма]</i>"
        )
    ...
    def clan_top_command(self):
        ...
        for i, clan in enumerate(clans, 1):
            ...
            top_text += (
                ...
                f"   🎯 Бонус: +{business_bonus}% от бизнесов в казну\n\n"
            )
        ...
        if result['success']:
            ...
            return jsonify({
                ...
                'message': f'💰 <b>Деньги внесены в казну клана!</b>\n\n'
                          ...
                          f'📈 <i>Деньги из казны можно использовать для улучшения клана или распределения между участниками</i>'
            })
        else:
            return jsonify({
                'success': False,
    ...
    def get_dumbbell_info(self, user_id):
        ...
        if clan:
            clan_bonuses = self.get_clan_bonuses(clan['level'])
            clan_bonus_text = f"\n🏰 <b>Бонус клана:</b> +{clan_bonuses['lift_bonus_coins']} монет за поднятие"
        ...
        info_text = (
            ...
            f"💰 Доход за подход: <b>{income_per_use} монет</b>{clan_bonus_text}\n"
    ...
    def get_profile(self, user_id):
        ...
        if clan:
            ...
            clan_bonus_text = f"🏰 Бонус клана: <b>+{clan_bonuses['lift_bonus_coins']} монет за поднятие</b>\n"
    ...
    def get_help(self):
        commands = [
            ...
            "💡 <b>Особенности новой системы кланов:</b>",
    ...
    # ======================
    # БИЗНЕС КОМАНДЫ (обновлены для новой системы)
    # ======================
    ...
    def show_all_businesses(self, user_id):
        ...
        clan = self.db.get_player_clan(user_id)
        if clan:
            clan_info = f"\n🏰 <b>Ваш клан:</b> [{clan['tag']}] {clan['name']}\n💰 <b>В казну клана:</b> ~{format_number(total_clan_income)} магнезии/час"
        ...
        info_text = (
            ...
            "\n\n".join(business_list) +
            f"{clan_info}\n\n"
            f"💎 <b>Общий баланс магнезии:</b> {format_number(player['magnesia'])} банок\n"
    ...
    def buy_business_command(self, user_id, business_id):
        ...
        if clan:
            clan_bonuses = self.get_clan_bonuses(clan['level'])
            clan_bonus_text = f"\n🏰 <b>Бонус клана:</b> +{clan_bonuses['business_bonus_percent']}% в казну клана"
    ...
    def admin_clan_info_command(self, user_id, tag):
        ...
        log_text = ""
        for entry in log:
            action_emoji = "➕" if entry['action_type'] == 'deposit' else ("⬆️" if entry['action_type'] == 'upgrade' else ("💰" if entry['action_type'] == 'lift_income' else ("🏦" if entry['action_type'] == 'business_income' else ("📊" if entry['action_type'] == 'distribution' else "📝"))))
            username = entry['username'] or "Система"
            time_str = datetime.fromisoformat(entry['created_at']).strftime("%d.%m %H:%M")
    ...
    def bot_statistics_command(self, user_id):
        ...
        cursor.execute('SELECT SUM(total_income_per_hour) FROM clans')
        total_clan_income = cursor.fetchone()[0] or 0
        ...
        stats_text = (
            ...
            f"🏰 <b>Кланы (НОВАЯ СИСТЕМА):</b>\n"
            f"├─ Всего кланов: <b>{total_clans}</b>\n"
            f"├─ Общая казна: <b>{format_number(total_clan_treasury)} монет</b>\n"
            f"└─ Общий доход/час: <b>{format_number(total_clan_income)} магнезии</b>\n\n"
            f"🎫 <b>Промокоды:</b>\n"
